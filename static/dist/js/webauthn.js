...
function checkUserExists() {
    $.get('http://localhost:9005/user/' + state.user.name + '/exists', {}, null, 'json')
...
function getCredentials() {
    $.get('http://localhost:9005/credential/' + state.user.name, {}, null, 'json')
        .done(function (response) {
...
function makeCredential() {
    ...
    $.get('http://localhost:9005/makeCredential/' + state.user.name, {
...
function registerNewCredential(newCredential) {
    ...
    $.ajax({
        url: 'http://localhost:9005/makeCredential',
        type: 'POST',
        data: JSON.stringify({
            id: newCredential.id,
...
function getAssertion() {
    ...
    setUser();
    $.get('http://localhost:9005/user/' + state.user.name + '/exists', {}, null, 'json').done(function (response) {
            console.log(response);
        }).then(function () {
            ...
            $.get('http://localhost:9005/assertion/' + state.user.name, {
...
function verifyAssertion(assertedCredential) {
    ...
    $.ajax({
        url: 'http://localhost:9005/assertion',
        type: 'POST',
        data: JSON.stringify({
